...
class CacheImpl implements Cache {
  ...
  Future<List<T>> _fetchT<T>(final String path, final String file, final Serializer serializer) async {
    ...
    if (await jsonFile.exists()) {
      ...
      if (jsonStr.isNotEmpty) {
        list =
            List.from(json.decode(jsonStr)).map((item) => serializers.deserializeWith(serializer, item) as T).toList();
      }
    }
  ...
  @override
  Future<bool> saveItem<T>(DatasourceType dataSourceType, T item) => saveItems(dataSourceType, [item]);
}

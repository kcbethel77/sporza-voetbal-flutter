...
void main() {
  ...
  when(cache.getT<Competition>(argThat(
          equals(CalendarForCompetitionDataSourceType(_competitionId)))))
      .thenAnswer((_) => Observable.empty());

  MockSporzaSoccerDataSource network = MockSporzaSoccerDataSource();

  CalendarForCompetitionUseCase calendarForCompetitionUseCase =
      CalendarForCompetitionUseCase(_competitionId, cache, network);

  group("calendar for competition use case", () {
    test("simple test", () {
      Competition competition = buildCompetition();
      when(network.getT<Competition>(any))
          .thenAnswer((_) => Observable.just(competition));

      calendarForCompetitionUseCase.calendarForCompetition
          .listen(expectAsync1((resp) {
